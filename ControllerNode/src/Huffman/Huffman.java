...
public class Huffman {
    ...
    /*
     * Descripción de la función: encapsule el proceso de codificación de Huffman
     * para llamar fácilmente
     * 
     * @param: contentbytes La matriz de bytes correspondiente al contenido a
     * transmitir
     * 
     * @return byte [] La cadena larga binaria completa de Huffman se convierte a
     * forma de byte para su transmisión
     */

    public static byte[] huffmanZip(byte[] contentbytes) {
        List<treeNode> contentlist = getList(contentbytes);// Convierta la matriz de bytes en una Lista compuesta por
                                                           // Nodo
     ...
     * @param dstFile ¿Dónde colocamos el archivo comprimido después de la
     *                compresión?
     */
    public static void zipFile(String srcFile, String dstFile) {
        ...
        try {
            ...
            byte[] huffmanBytes = huffmanZip(b);// Obtenga la cadena larga binaria completa de Huffman y conviértala en
                                                // forma de byte para su transmisión
            ...
            oos.writeObject(huffmanCodes);// Tenga cuidado aquí, debe pasar la tabla de mapeo de codificación de
                                          // Huffman, de lo contrario no se puede descomprimir

        } catch (Exception e) {
            System.out.println(e.getMessage());
        } finally {
            try {
                oos.close();// El orden de cierre de la transmisión debe ser opuesto al orden de creación de
                            // la transmisión
                os.close();
                is.close();
            } catch (Exception e) {
    ...
    public static List<treeNode> getList(byte[] contentbytes) {
        ...
        // Convierta cada par de valores clave en treeNode y almacénelos en la colección
        // de nodos
        // Recorre el mapa
    ...
    public static treeNode createhuffmantree(List<treeNode> nodes) {
        while (nodes.size() > 1) {
            ...
            treeNode parentNode = new treeNode(null, leftNode.value + rightNode.value);// Los nodos que no son hojas
                                                                                       // solo tienen pesos, sin datos
    ...
    /*
     * Descripción de la función: convierta el árbol Huffman en código Huffman y
     * guárdelo en el mapa
     * 
    ...
    /*
     * Descripción de la función: sobrecargue la función getcdes (haga que el
     * formulario de referencia sea más conciso)
     * 
    ...
    /*
     * Descripción de la función: la matriz de bytes [] correspondiente a la cadena
     * se genera de acuerdo con el mapa de código de Huffman, y una cadena larga de
     * códigos binarios de Huffman se genera y se convierte en byte []
     * 
    ...
    public static byte[] zip(byte[] bytes, Map<Byte, String> huffmanCodes) {
        ...
        for (int i = 0; i < stringBuilder.length(); i = i + 8) {
            ...
            if (stringBuilder.length() < i + 8) {
                ...
                curString = stringBuilder.substring(i, i + 8);// índice de coordenadas de subcadena antes y después no
                                                              // incluido
            }
            huffmanCodesByte[index] = (byte) Integer.parseInt(curString, 2);// Por defecto, curString está en formato
                                                                            // binario, conviértalo a decimal Int, y
                                                                            // luego convierta el tipo a byte
            index++;
        }
     ...
     * 
     * @param: si la bandera debe complementarse con bits altos, si es verdadera,
     * debe complementarse, y false no. Si es el último byte, no es necesario llenar
     * el bit alto
     * 
     * @param: b
     * 
     * @return java.lang.String es la cadena binaria correspondiente ab (tenga en
     * cuenta que se devuelve en complemento)
     */
    ...
    public static byte[] decode(Map<Byte, String> huffmanCodes, byte[] huffmanCodesByte) {
        ...
        // 1. Reemplace la tabla de asignación de codificación de Huffman, porque se
        // requiere una consulta inversa
        Map<String, Byte> map = new HashMap<String, Byte>();// la clave es una cadena binaria, el valor es el byte
                                                            // correspondiente al carácter decodificado
        ...
        for (int i = 0; i < stringBuilder1.length();) {
            int count = 0;// Contador interno, debido a que el número de bits de Huffman correspondientes
                          // a cada carácter escaneado es incierto, el contador se borra cada vez que la
                          // búsqueda es exitosa
            boolean flag = true;
            while (flag) {
